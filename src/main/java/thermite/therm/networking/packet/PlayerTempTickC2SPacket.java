...
public class PlayerTempTickC2SPacket {
    ...
    public static void receive(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
        ...
        if ((int)playerState.restingTemp > (int)playerState.temp) { //TODO fix temprate
            playerState.temp += 0.125;
            tempDir = 1;
        } else if ((int)playerState.restingTemp < (int)playerState.temp) {
            playerState.temp -= 0.125;
            tempDir = -1;
        } else if ((int)playerState.restingTemp == (int)playerState.temp) {tempDir = 0;}
